//
//  TSDataPersistenceLinkedList.swift
//  TSDataPersistence
//
...
protocol TSDataPersistenceLRUObject {
    
    var key: String { get }
    var cost: UInt { get set }
}

class TSDataPersistenceLRUGenerator<T: TSDataPersistenceLRUObject> : FastGeneratorType {
...
class LRU<T: TSDataPersistenceLRUObject> {
...
extension LRU : Sequence {
    ...
    typealias Iterator = TSDataPersistenceLRUGenerator<T>
    
    
    func makeIterator() -> TSDataPersistenceLRUGenerator<T> {
        var generatror: TSDataPersistenceLRUGenerator<T>
        generatror = TSDataPersistenceLRUGenerator(linkedListGenerator: _linkedList.makeIterator(), lru: self)
        return generatror
    }
}
