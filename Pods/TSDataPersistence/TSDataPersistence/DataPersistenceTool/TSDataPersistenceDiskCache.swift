//
//  TSDataPersistenceDiskCache.swift
//  TSDataPersistence
//
 ...
 */
open class TSDataPersistenceDiskCacheGenerator : FastGeneratorType {
    
    public typealias Element = (String, Any)
    
    fileprivate var _lruGenerator: TSDataPersistenceLRUGenerator<TSDataPersistenceDiskCacheObject>
    
    fileprivate var _diskCache: TSDataPersistenceDiskCache
    
    fileprivate var _completion: (() -> Void)?
    
    fileprivate init(generate: TSDataPersistenceLRUGenerator<TSDataPersistenceDiskCacheObject>, diskCache: TSDataPersistenceDiskCache, completion: (() -> Void)?) {
...
private class TSDataPersistenceDiskCacheObject: TSDataPersistenceLRUObject {
...
public typealias DiskCacheAsyncCompletion = (_ cache: TSDataPersistenceDiskCache?, _ key: String?, _ object: Any?) -> Void
 ...
 */
open class TSDataPersistenceDiskCache {
    ...
    fileprivate let _cache: LRU = LRU<TSDataPersistenceDiskCacheObject>()
    
    fileprivate let _queue: DispatchQueue = DispatchQueue(label: TrackCachePrefix + (String(describing: TSDataPersistenceDiskCache.self)), attributes: DispatchQueue.Attributes.concurrent)
     ...
     */
    open static let shareInstance = TSDataPersistenceDiskCache(name: TrackCacheDefauleName)!
...
//  MARK:
//  MARK: Public
public extension TSDataPersistenceDiskCache {
    //  MARK: Async
    /**
     ...
     */
    public func set<T:Codable>(_ object: T, forKey key: String, completion: DiskCacheAsyncCompletion?) {
    ...
    public func set<T:Codable>(_ object: T, forKey key: String) {
        ...
        if NSKeyedArchiver.archiveRootObject(object, toFile: filePath) == true {
            do {
                ...
                _cache.set(object: TSDataPersistenceDiskCacheObject(key: key, cost: fileSize, date: date), forKey: key)
            } catch {}
        }
...
//  MARK: SequenceType
extension TSDataPersistenceDiskCache : Sequence {
    /**
     MemoryCacheGenerator
     */
    public typealias Iterator = TSDataPersistenceDiskCacheGenerator
    ...
    public func makeIterator() -> TSDataPersistenceDiskCacheGenerator {
        var generatror: TSDataPersistenceDiskCacheGenerator
        _lock()
        generatror = TSDataPersistenceDiskCacheGenerator(generate: _cache.makeIterator(), diskCache: self) {
            self._unlock()
        }
        return generatror
    }
...
//  MARK:
//  MARK: Private
private extension TSDataPersistenceDiskCache {
    ...
    func _loadFilesInfo() -> Bool {
        var fileInfos: [TSDataPersistenceDiskCacheObject] = [TSDataPersistenceDiskCacheObject]()
            ...
            for fileURL: URL in filesURL {
                do {
                    ...
                    if let key = fileURL.lastPathComponent as String?,
                        ...
                        fileInfos.append(TSDataPersistenceDiskCacheObject(key: key, cost: fileSize.uintValue, date: date))
                    }
                }
                catch {
                    return false
    ...
    func _unsafeTrim(toCount countLimit: UInt) {
        if var lastObject: TSDataPersistenceDiskCacheObject = _cache.lastObject() {
